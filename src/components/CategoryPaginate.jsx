...
const CategoryPaginate = () => {
  const isLoggedIn = useSelector(getIsLoggedIn);
  const blogStatus = useSelector(getBlogStatus);

  const { last_page } = useSelector(getCategoryDetails);
  ...
  useEffect(() => {
    dispatch(obtainCategoryBlogs(currentPage, categoryName));
    if (last_page) {
      setTotalItems(last_page);
    }
  }, [currentPage, last_page, dispatch, categoryName]);
